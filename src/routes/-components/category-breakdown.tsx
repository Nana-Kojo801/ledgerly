...
import { mockDashboardData, getColorValue, formatCurrency } from "../-mock-data.ts";
...
export function CategoryBreakdown() {
  ...
  const { categories } = mockDashboardData;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className="p-4 rounded-lg bg-surface-1 border border-border/50">
          ...
          <div className="text-2xl font-bold">{sortedCategories[0]?.percentage}%</div>
        </div>
        <div className="p-4 rounded-lg bg-surface-1 border border-border/50">
          <div className="text-sm font-medium text-muted-foreground">Daily Avg</div>
          <div className="text-2xl font-bold">${(totalSpent / 30).toFixed(2)}</div>
        </div>
      </div>
